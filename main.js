...
d3.csv("nobel_laureates.csv").then(data => {
    ...
    // 4: SET SCALES
    // Set up variables
    // Getting the extent of all years from the rolled-up structure
    const allYears = Array.from(categories.values())
        .flatMap(yearMap => Array.from(yearMap.keys()));

    // Getting the max count from the rolled-up structure
    ...
    // Set up the color scale using categories from the rolled-up data
    ...
    // 5: PLOT LINES
    // Convert the Map to an array of [category, yearMap] pairs
    ...
    // - Add a "class" to each line for styling.
        .attr("d", d => {
            const category = d[0];  // Category name (e.g., "STEM" or "Non-STEM")
